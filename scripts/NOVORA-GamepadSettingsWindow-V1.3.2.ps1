...
# =====================================================================
# NOVORA-LINK
# VISIONENGINE - GAMEPAD SETTINGS WINDOW V1.3.2
#
# Compatible con:
...
#   - Acceso desde SettingsWindow mediante un solo boton.
#   - Activar/desactivar GamepadVE.
#   - Seleccion de mando fisico preferido por nombre SDL.
#   - Seleccion "Automatico" para conservar el comportamiento actual.
#   - Diagnostico SDL3 bajo demanda (SIN polling nuevo).
#   - Medicion local del coste de lectura SDL (NO es latencia Android).
#   - Persistencia en el settings.json actual de NOVORA.
#   - ManagerGamepadVE respeta Enabled + mando preferido al iniciar.
#   - Pruebas de integracion al final, despues de escribir las fuentes.
#   - Preflight Rebuild para descartar obj/WPF incremental obsoleto.
#   - Limpieza de obj antes de GREEN y despues de rollback.
#   - Backup + rollback automatico.
#   - Proteccion SHA-256 de LinkEngine y Android LinkEngine.
#
# NO IMPLEMENTA TODAVIA:
#   - Keyboard + Mouse -> Gamepad (siguiente bloque).
#   - Rumble Android -> mando fisico.
#   - LED DualSense/DualShock.
#   - Touchpad/Gyro/Acelerometro.
#   - Adaptive Triggers.
#   - Eliminacion del polling historico de ManagerGamepadVE.
#
# IMPORTANTE:
#   Este V1.3.2 NO agrega timers ni polling al menu. La deteccion/diagnostico
...
function Replace-ExactlyOnce
{
    ...
    if ($first -lt 0)
    {
        throw "No encontre el bloque esperado: $Description"
    }
    ...
    if ($second -ge 0)
    {
        throw "El bloque aparece mas de una vez: $Description"
    }
...
{
    throw @"
La ventana/configuracion de GamepadVE ya parece existir.
...
# El diagnostico del 2026-09-07 confirmo que SettingsWindow.g.cs podia
# quedar obsoleto dentro de obj aunque SettingsWindow.xaml ya estuviera
# limpio. Por eso V1.3.2 nunca confia en obj heredado.
...
if ($Baseline.ExitCode -ne 0)
{
    ...
    throw @"
El baseline de NOVORA Windows NO compila despues de limpiar obj y ejecutar Rebuild.

No se ha modificado ninguna fuente de Gamepad Settings.
El proyecto Android LinkEngine NO forma parte de este gate.
...
Write-Host "[OK] Baseline NOVORA Windows limpio: Rebuild Release paso." -ForegroundColor Green
...
$BackupRoot = Join-Path $env:TEMP "NOVORA-GAMEPAD-SETTINGS-V1.3.2-$Timestamp"
...
try
{
    ...
    Write-Section "APLICANDO GAMEPAD SETTINGS WINDOW V1.3.2"

    $IntroMessageVE = @"
V1.3.2 aplica primero la integracion y ejecuta las pruebas al final.
No se usa TDD RED previo porque el baseline WPF ya demostro que puede
conservar artefactos .g.cs obsoletos en obj.
"@

    Write-Host $IntroMessageVE -ForegroundColor DarkGray
...
/// <summary>
/// Configuracion persistente minima de GamepadVE.
///
/// V1.3.2 solo gobierna:
/// - habilitar/deshabilitar GamepadVE;
/// - seleccionar el mando fisico preferido.
///
/// No almacena seriales, cuentas ni informacion personal.
/// </summary>
...
'@
    ...
    /// <summary>
    /// Permite desactivar completamente la creacion de gamepads UHID
    /// sin afectar teclado/mouse normal de ControlVE.
    /// </summary>
    public bool GamepadEnabled { get; set; } = true;

    /// <summary>
    /// Nombre SDL del mando fisico preferido.
    /// "__auto__" conserva la deteccion automatica actual.
    ...
    <Border
        ...
        <Grid>
            ...
            <StackPanel Grid.Row="0" Margin="0,0,0,18">
                ...
                <TextBlock
                    Text="Configura el gamepad de VisionEngine sin llenar la configuracion general de NOVORA."
                    Margin="0,5,0,0"
                    FontSize="11"
                    ...
                    <Border
                        ...
                        <Grid>
                            ...
                            <StackPanel Grid.Row="1" Grid.Column="0" Margin="0,0,18,14">
                                ...
                                <TextBlock
                                    Text="Selecciona un mando o deja automatico"
                                    Style="{StaticResource GamepadCaption}"
                                    Margin="0,3,0,0"/>
                            </StackPanel>
                            ...
                            <StackPanel Grid.Row="2" Grid.Column="0" Margin="0,0,18,0">
                                <TextBlock Text="Deteccion" Style="{StaticResource GamepadTitle}"/>
                                <TextBlock
                                    Text="Consulta SDL solo cuando tu lo pides"
                                    Style="{StaticResource GamepadCaption}"
                                    Margin="0,3,0,0"/>
                            </StackPanel>
                    ...
                    <Border
                        ...
                        <Grid>
                            ...
                            <TextBlock
                                ...
                                Grid.Row="0"
                                Text="SDL3 todavia no comprobado."
                                Foreground="White"
                                FontSize="12"
                                TextWrapping="Wrap"/>
                    ...
                    <Border
                        ...
                        <Grid>
                            ...
                            <StackPanel Grid.Column="0" Margin="0,0,18,0">
                                ...
                                <TextBlock
                                    Margin="0,5,0,0"
                                    Text="Aqui conectaremos WASD al stick izquierdo y movimiento relativo del raton al stick derecho. Esta V1 solo prepara la ventana; el mapper se implementa en el siguiente bloque para no mezclar dos cambios grandes."
                                    Foreground="{DynamicResource MutedBrush}"
                                    FontSize="11"
                                    TextWrapping="Wrap"
                                    LineHeight="17"/>
                    ...
                    <Border
                        ...
                        <Grid>
                            ...
                            <WrapPanel Grid.Row="1" Margin="0,12,0,0">
                                <CheckBox Content="Vibracion" IsEnabled="False" IsChecked="True" Foreground="White" Margin="0,0,22,8"/>
                                ...
                                <CheckBox Content="Acelerometro" IsEnabled="False" Foreground="White" Margin="0,0,22,8"/>

                                <TextBlock
                                    Width="620"
                                    Margin="0,5,0,0"
                                    Text="Estas opciones se habilitaran cuando conectemos UhidOutputVE con las capacidades reales de SDL/DualSense. Se muestran desactivadas para no prometer funciones que todavia no ejecuta el backend."
                                    Foreground="{DynamicResource MutedBrush}"
                                    FontSize="10"
                                    TextWrapping="Wrap"/>
                            </WrapPanel>
            ...
            <Grid Grid.Row="2" Margin="0,18,0,0">
                ...
                <TextBlock
                    Grid.Column="0"
                    Text="Los cambios de mando se aplican al iniciar la siguiente sesion de VisionEngine."
                    FontSize="10"
                    Foreground="{DynamicResource MutedBrush}"
                    VerticalAlignment="Center"
                    TextWrapping="Wrap"/>
    ...
    private void RefreshGamepadsVE()
    {
        ...
        _optionsVE.Add(
            new OptionGamepadVE(
                null,
                ConfigGamepadVE.AutoSelectionVE,
                "Automatico - usar mandos detectados"));
        ...
        try
        {
                ...
                selected.InstanceIdVE is null)
            {
                DiagnosticStatusText.Text =
                    $"El mando seleccionado no esta conectado: {selected.LabelVE}";
            ...
            {
                DiagnosticStatusText.Text =
                    "No hay un mando fisico disponible para probar.";
            ...
            {
                DiagnosticStatusText.Text =
                    "SDL3 detecto el mando, pero no pudo abrirlo.";
            ...
            try
            {
                ...
                DiagnosticStatusText.Text =
                    ...
                    "Esta medicion cubre SDL local en Windows; no es la latencia completa hasta Android/juego.";
            }
            finally
            {
                _sdlVE.CloseVE(
...
'@
    ...
    # =================================================================
    # 5. SETTINGS WINDOW - BOTON DE ENTRADA
    # =================================================================
                    ...
                    <Border
                        ...
                        <Grid>
                            ...
                            <StackPanel Grid.Column="0" Margin="0,0,18,0">
                                ...
                                <TextBlock
                                    Margin="0,5,0,0"
                                    Text="Selecciona el mando, comprueba SDL3 y prepara perfiles de teclado/raton en una ventana independiente."
...
'@
    ...
    /*
     * Configuracion leida UNA sola vez al iniciar GamepadVE.
...
'@
    ...
    $ManagerGamepad = Replace-ExactlyOnce `
        ...
        -Old $OldFields `
        -New $NewFields `
        -Description "Campos de configuracion GamepadVE"
...
'@
    ...
    $OldPublish = @'
                    ...
                    _ when _slotsVE.Count == 0 =>
                        "Gamepad VisionEngine activo; esperando control fisico.",
'@
    ...
    $NewPublish = @'
                    ...
                    _ when !_enabledBySettingsVE =>
                        "Gamepad VisionEngine desactivado desde Configuracion.",
...
'@
    ...
    $ManagerGamepad = Replace-ExactlyOnce `
        ...
        -Old $OldPublish `
        -New $NewPublish `
        -Description "Estado de seleccion GamepadVE"

    Write-TextUtf8NoBom -Path $ManagerGamepadPath -Content $ManagerGamepad
    Write-Host "[OK] ManagerGamepadVE respeta configuracion al iniciar." -ForegroundColor Green

    # =================================================================
    # 7. VALIDACION XAML + PRUEBAS + BUILD
    # =================================================================

    Write-Section "7/7 - VALIDACION FINAL"

    try
    {
        [xml](Get-Content -LiteralPath $SettingsWindowXaml -Raw) | Out-Null
        [xml](Get-Content -LiteralPath $GamepadWindowXaml -Raw) | Out-Null
        Write-Host "[OK] XAML valido como XML." -ForegroundColor Green
    }
    catch
    {
        throw "XAML invalido: $($_.Exception.Message)"
    }

    # =================================================================
    # VERIFICACION FISICA DE LA INTEGRACION
    # =================================================================

    Write-Host ""
    Write-Host "=== VERIFICACION FISICA EN DISCO ===" -ForegroundColor Cyan
    ...
    foreach ($requiredFileVE in $RequiredFilesVE)
    {
        if (-not (
            Test-Path `
                -LiteralPath $requiredFileVE `
                -PathType Leaf
        ))
        {
            throw "INTEGRACION FISICA INCOMPLETA: falta $requiredFileVE"
        }
    }

    $VerifySettingsXamlVE = Read-Text $SettingsWindowXaml
    $VerifySettingsCodeVE = Read-Text $SettingsWindowCode
    $VerifySettingsServiceVE = Read-Text $SettingsServicePath
    $VerifyManagerVE = Read-Text $ManagerGamepadPath
    $VerifyWindowXamlVE = Read-Text $GamepadWindowXaml
    $VerifyWindowCodeVE = Read-Text $GamepadWindowCode
    $VerifyConfigVE = Read-Text $ConfigGamepadPath

    $ChecksVE = @(
        [pscustomobject]@{
            Name = "SettingsWindow: seccion CONTROLES"
            Passed = $VerifySettingsXamlVE.Contains('<!-- CONTROLES -->')
        },
        [pscustomobject]@{
            Name = "SettingsWindow: boton CONFIGURAR"
            Passed = $VerifySettingsXamlVE.Contains('Click="GamepadSettings_Click"')
        },
        [pscustomobject]@{
            Name = "SettingsWindow.xaml.cs: handler"
            Passed = $VerifySettingsCodeVE.Contains('GamepadSettings_Click(')
        },
        [pscustomobject]@{
            Name = "GamepadSettingsWindow XAML"
            Passed = $VerifyWindowXamlVE.Contains('x:Class="NOVORA.GamepadSettingsWindow"')
        },
        [pscustomobject]@{
            Name = "GamepadSettingsWindow code-behind"
            Passed = $VerifyWindowCodeVE.Contains('public partial class GamepadSettingsWindow : Window')
        },
        [pscustomobject]@{
            Name = "ConfigGamepadVE"
            Passed = $VerifyConfigVE.Contains('public sealed record ConfigGamepadVE')
        },
        [pscustomobject]@{
            Name = "Settings: GamepadEnabled"
            Passed = $VerifySettingsServiceVE.Contains('public bool GamepadEnabled')
        },
        [pscustomobject]@{
            Name = "Settings: SelectedGamepadName"
            Passed = $VerifySettingsServiceVE.Contains('public string SelectedGamepadName')
        },
        [pscustomobject]@{
            Name = "ManagerGamepadVE: enabled"
            Passed = $VerifyManagerVE.Contains('_enabledBySettingsVE')
        },
        [pscustomobject]@{
            Name = "ManagerGamepadVE: seleccion preferida"
            Passed = $VerifyManagerVE.Contains('_preferredPhysicalNameVE')
        },
        [pscustomobject]@{
            Name = "ManagerGamepadVE: filtro fisico"
            Passed = $VerifyManagerVE.Contains('ConfigGamepadVE.MatchesSelectionVE(')
        }
    )
    ...
    if ($FailedChecksVE.Count -ne 0)
    {
        throw @"
La integracion no quedo fisicamente escrita en disco.
No continuare a tests/build con una integracion incompleta.
"@
    }

    Write-Host "[OK] Integracion fisica confirmada ANTES de compilar." -ForegroundColor Green

    # Pruebas al final, como se pidio para este proyecto.
    $GamepadTests = @'
using NOVORA.Services;
...
'@
    ...
    Write-TextUtf8NoBom `
        -Path $GamepadTestsPath `
        -Content $GamepadTests
    ...
    try
    {
        ...
        Write-Host ""
        Write-Host "=== PRUEBAS GAMEPAD SETTINGS ===" -ForegroundColor Cyan
        ...
        if ($Green.ExitCode -ne 0)
        {
            $GreenFailurePath =
                Join-Path `
                    $env:USERPROFILE `
                    (
                        "Desktop\NOVORA-GAMEPADSETTINGS-TESTS-FAIL-" +
                        (Get-Date -Format "yyyyMMdd-HHmmss") +
                        ".txt"
                    )
...
La salida completa quedo guardada en:
$GreenFailurePath
"@
        }

        Write-Host "[OK] GamepadSettingsTests pasaron." -ForegroundColor Green
        ...
        {
            throw "NOVORA Windows no compilo en Release."
        }
    }
    finally
    {
    ...
    {
        throw "INTEGRIDAD: LinkEngine cambio durante una integracion exclusiva de VisionEngine/Gamepad."
    }

    if ($AndroidLinkAfter -ne $AndroidLinkBefore)
    {
        throw "INTEGRIDAD: Android LinkEngine cambio durante una integracion exclusiva de VisionEngine/Gamepad."
    }

    Write-Host "[OK] LinkEngine intacto." -ForegroundColor Green
    Write-Host "[OK] Android LinkEngine intacto." -ForegroundColor Green

    # Verificacion posterior al Rebuild: fuentes deben seguir presentes.
    if (
        -not (Test-Path -LiteralPath $GamepadWindowXaml -PathType Leaf) -or
        -not (Test-Path -LiteralPath $GamepadWindowCode -PathType Leaf) -or
        -not (Test-Path -LiteralPath $ConfigGamepadPath -PathType Leaf) -or
        -not (Read-Text $SettingsWindowXaml).Contains('Click="GamepadSettings_Click"') -or
        -not (Read-Text $SettingsWindowCode).Contains('GamepadSettings_Click(')
    )
    {
        throw "POST-BUILD: la integracion desaparecio o quedo incompleta."
    }
    ...
    $SuccessReportVE =
        Join-Path `
            $env:USERPROFILE `
            (
                "Desktop\NOVORA-GAMEPADSETTINGS-INTEGRATED-V1.3.2-" +
                (Get-Date -Format "yyyyMMdd-HHmmss") +
                ".txt"
            )
    ...
    $SuccessContentVE = @"
NOVORA-LINK
GAMEPAD SETTINGS WINDOW V1.3.2
...
SettingsWindow:
CONTROLES = OK
GamepadSettings_Click = OK

Settings:
GamepadEnabled = OK
SelectedGamepadName = OK

ManagerGamepadVE:
seleccion preferida = OK

Tests:
GamepadSettingsTests = OK

Build:
src\NOVORA\NOVORA.csproj Release Rebuild = OK

Nota:
NOVORA.linkEngine.Android no se compila como gate de esta integracion
porque pertenece a LinkEngine y presenta errores independientes.
...
"@
    ...
    Write-Host ""
    Write-Host "[OK] Reporte de integracion:" -ForegroundColor Green
    Write-Host $SuccessReportVE -ForegroundColor Green

    Write-Section "GAMEPAD SETTINGS WINDOW V1.3.2 INTEGRADO"
...
Funcional ahora:
  ...
  [OK] Seleccionar mando fisico preferido
  [OK] Automatico
  [OK] Deteccion SDL bajo demanda
  [OK] Diagnostico SDL local bajo demanda
  [OK] Persistencia settings.json
  [OK] ManagerGamepadVE respeta la seleccion al iniciar
...
"@
    ...
    Write-Host $FinalMessageVE -ForegroundColor Green
}
catch
{
    ...
    # Nunca dejamos .g.cs/.baml generados por una integracion fallida.
    try
    {
        Remove-GeneratedArtifacts
    }
    ...
    try
    {
        ...
        {
            Write-Host "[ROLLBACK] Baseline NOVORA Windows restaurado y Rebuild correcto." -ForegroundColor Green
        }
        else
        {
            $RollbackFailurePath =
            ...
            Write-Host "[ROLLBACK] El Rebuild del baseline restaurado fallo." -ForegroundColor Red
            Write-Host "[ROLLBACK] Diagnostico: $RollbackFailurePath" -ForegroundColor Red
        }
    }
    catch
    {
    ...
    Write-Host ""
    Write-Host "INTEGRACION CANCELADA:" -ForegroundColor Red
