...
# =====================================================================
# NOVORA-LINK
# VISIONENGINE - GAMEPAD SETTINGS WINDOW V1.3
#
# Compatible con:
...
#   - Pruebas de integración al final, después de escribir las fuentes.
...
#
# IMPORTANTE:
#   Este V1.3 NO agrega timers ni polling al menú. La detección/diagnóstico
...
# limpio. Por eso V1.3 nunca confía en obj heredado.
...
$BackupRoot = Join-Path $env:TEMP "NOVORA-GAMEPAD-SETTINGS-V1.3-$Timestamp"
...
try
{
    ...
    Write-Section "APLICANDO GAMEPAD SETTINGS WINDOW V1.3"

    Write-Host @"
V1.3 aplica primero la integración y ejecuta las pruebas al final.
No se usa TDD RED previo porque el baseline WPF ya demostró que puede
...
'@
    ...
    # =================================================================
    # 7. VALIDACIÓN XAML + PRUEBAS + BUILD
    # =================================================================
    ...
    # =================================================================
    # VERIFICACIÓN FÍSICA DE LA INTEGRACIÓN
    # =================================================================

    Write-Host ""
    Write-Host "=== VERIFICACIÓN FÍSICA EN DISCO ===" -ForegroundColor Cyan
    ...
    foreach ($requiredFileVE in $RequiredFilesVE)
    {
        ...
        {
            throw "INTEGRACIÓN FÍSICA INCOMPLETA: falta $requiredFileVE"
        }
    }
    ...
    $ChecksVE = @(
        [pscustomobject]@{
            Name = "SettingsWindow: sección CONTROLES"
            Passed = $VerifySettingsXamlVE.Contains('<!-- CONTROLES -->')
        },
        [pscustomobject]@{
            Name = "SettingsWindow: botón CONFIGURAR"
        ...
        [pscustomobject]@{
            Name = "ManagerGamepadVE: selección preferida"
            Passed = $VerifyManagerVE.Contains('_preferredPhysicalNameVE')
        },
        [pscustomobject]@{
            Name = "ManagerGamepadVE: filtro físico"
    ...
    {
        throw @"
La integración no quedó físicamente escrita en disco.
No continuaré a tests/build con una integración incompleta.
"@
    }

    Write-Host "[OK] Integración física confirmada ANTES de compilar." -ForegroundColor Green

    # Pruebas al final, como se pidió para este proyecto.
    $GamepadTests = @'
using NOVORA.Services;
...
"@
    ...
    # Verificación posterior al Rebuild: fuentes deben seguir presentes.
    if (
    ...
    {
        throw "POST-BUILD: la integración desapareció o quedó incompleta."
    }
    ...
    $SuccessReportVE =
        Join-Path `
            $env:USERPROFILE `
            (
                "Desktop\NOVORA-GAMEPADSETTINGS-INTEGRATED-V1.3-" +
                (Get-Date -Format "yyyyMMdd-HHmmss") +
                ".txt"
            )
...
ManagerGamepadVE:
selección preferida = OK

Tests:
GamepadSettingsTests = OK

Build:
NOVORA.sln Release Rebuild = OK
...
"@
    ...
    Write-Host ""
    Write-Host "[OK] Reporte de integración:" -ForegroundColor Green
    Write-Host $SuccessReportVE -ForegroundColor Green

    Write-Section "GAMEPAD SETTINGS WINDOW V1.3 INTEGRADO"
