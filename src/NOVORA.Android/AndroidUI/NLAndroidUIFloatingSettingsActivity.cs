...
public sealed class NLAndroidUIFloatingSettingsActivity : Activity
{
    ...
    private void Render()
    {
        ...
        var exclude = new Switch(this) { Text = "Ocultar burbuja en capturas (requiere comprobar en tu teléfono)", Checked = NLAndroidUIFloatingPreferences.ExcludeFromCapture(this) };
        exclude.SetTextColor(Color.White); exclude.SetPadding(0, Dp(12), 0, Dp(12));
        exclude.CheckedChange += (_, e) => NLAndroidUIFloatingPreferences.SetExcludeFromCapture(this, e.IsChecked);
        _body.AddView(exclude);
        Label("Experimental: solicitamos a Android excluir el panel abierto y, si activas esta opción, también la burbuja plegada. Comprueba una captura antes de grabar: el resultado puede no mostrar limpiamente el contenido debajo del control. La exclusión depende del teléfono y del mecanismo de captura.");
    ...
    private void AddButton(string text, Action action) { var button = new Button(this) { Text = text }; button.SetAllCaps(false); button.Click += (_, _) => action(); _body.AddView(button, new LinearLayout.LayoutParams(-1, -2)); }
    private Button SmallButton(LinearLayout row, string text, Action action) { var button = new Button(this) { Text = text }; button.SetAllCaps(false); button.Click += (_, _) => action(); row.AddView(button, new LinearLayout.LayoutParams(0, -2, 1)); return button; }
