...
public sealed class VEAudioManager : IAsyncDisposable
{
    ...
    private VEAudioStatus _statusVE =
        VEAudioStatus.CreateInitialVE();

    private CancellationTokenSource? _ctsVE;
    private Task? _taskVE;

    private VEAudioDecoder? _decoderVE;
    private VEAudioPlayer? _playerVE;
    ...
    public event EventHandler<VEAudioStatus>?
        StatusChangedVE;

    public event EventHandler<VEAudioFrame>?
        FrameDecodedVE;

    public VEAudioStatus StatusVE
    {
        get
        {
            lock (_gateVE)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (
            codec ==
            VEProtocolCodec.Disabled
        )
        {
            PublishVE(
                VEAudioStates.Disabled,
                codec,
                null,
                false,
        ...
        if (
            codec ==
            VEProtocolCodec.ConfigurationError
        )
        {
            throw new InvalidOperationException(
                    ...
                    .IsCancellationRequested)
            {
                    ...
                    VEAudioFrame frame in frames)
                {
                    ...
                    )
                    {
                        ...
                        {
                            /*
                             * El mismo objeto VEAudioPlayer continúa existiendo
    ...
    private VEAudioStats SnapshotVE()
        => new(
            Interlocked.Read(
                ref _packetsVE),
