...
/// </summary>
public sealed class VEAudioDecoder : IDisposable
{
    ...
    public VEAudioDecoder(NLServiceNovoraPaths paths) => _paths = paths ?? throw new ArgumentNullException(nameof(paths));

    public void InitializeVE(VEProtocolCodec codec)
    {
        ThrowIfDisposedVE();
        if (_contextVE != IntPtr.Zero)
        {
            if (_codecVE == codec) return;
            throw new InvalidOperationException("VEAudioDecoder ya fue inicializado con otro codec.");
        }
    ...
    public IReadOnlyList<VEAudioFrame> DecodeVE(VEAudioPacket packet)
    {
        ThrowIfDisposedVE();
        ArgumentNullException.ThrowIfNull(packet);
        if (_contextVE == IntPtr.Zero) throw new InvalidOperationException("VEAudioDecoder no está inicializado.");
        if (packet.IsConfiguration) return Array.Empty<VEAudioFrame>();

        IntPtr buffer = _mallocVE!(checked((nuint)(packet.Data.Length + VEProtocolConstants.FfmpegInputPaddingSizeVE)));
        ...
        try
        {
            ...
            byte[] padding = new byte[VEProtocolConstants.FfmpegInputPaddingSizeVE];
            ...
            if (result < 0 && result != VEProtocolConstants.FfmpegAgainVE) throw ErrorVE("avcodec_send_packet", result);
            List<VEAudioFrame> frames = [];
            long decodedSamples = 0;
            while (true)
            {
                result = _receiveFrameVE!(_contextVE, _frameVE);
                if (result == VEProtocolConstants.FfmpegAgainVE || result == VEProtocolConstants.FfmpegEofVE) break;
                if (result < 0) throw ErrorVE("avcodec_receive_frame", result);
                try
                {
                    long? timestamp = packet.PresentationTimeUs is { } pts
                        ? checked(pts + decodedSamples * 1_000_000L / SampleRateVE) : null;
                    VEAudioFrame converted = ConvertFrameVE(_frameVE, timestamp);
                    frames.Add(converted);
                    decodedSamples += converted.SamplesPerChannel;
                }
    ...
    private VEAudioFrame ConvertFrameVE(IntPtr frame, long? pts)
    {
