...
/// Reutiliza NLServiceADB: VisionEngine no crea un segundo mecanismo de polling ADB.
/// </summary>
public sealed partial class VEDeviceManager
{
    private readonly NLServiceADB _adb;
    private readonly object _statusGate = new();

    private VEDeviceStatus _status =
        VEDeviceStatus.CreateInitialVE();

    public VEDeviceManager(NLServiceADB adb)
    {
        _adb = adb ?? throw new ArgumentNullException(nameof(adb));
    }

    public event EventHandler<VEDeviceStatus>? StatusChangedVE;

    public VEDeviceStatus StatusVE
    {
        get
        {
            lock (_statusGate)
    ...
    public async Task<VEDeviceSession> OpenAsync(
        string serial,
    ...
    private void PublishStatusVE(VEDeviceStatus status)
    {
        EventHandler<VEDeviceStatus>? handler;
