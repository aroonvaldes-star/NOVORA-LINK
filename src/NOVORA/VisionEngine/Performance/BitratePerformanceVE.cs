...
public sealed class BitratePerformanceVE
{
    ...
    public int RecommendVE(int currentBitrate, CongestionPerformanceVE congestion)
    {
        int current = Math.Clamp(currentBitrate, MinBitrateVE, MaxBitrateVE);
        double factor = congestion switch
        {
            CongestionPerformanceVE.Healthy => IncreaseFactorVE,
            CongestionPerformanceVE.Mild => 1.0,
            CongestionPerformanceVE.Moderate => 0.90,
            CongestionPerformanceVE.Severe => DecreaseFactorVE,
            CongestionPerformanceVE.Critical => Math.Min(DecreaseFactorVE, 0.60),
            _ => 1.0
        };
