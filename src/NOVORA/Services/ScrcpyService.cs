...
public sealed class ScrcpyService : IDisposable
{
    ...
    public Process StartOptimized(DeviceInfo device, MonitorInfo monitor, OutputProfile profile, bool audioEnabled = true)
    {
        ...
        if (_sessions.TryGetValue(serial, out var existing))
        {
            if (!existing.Process.HasExited) return existing.Process;
        ...
        if (!_sessions.TryAdd(serial, session))
        {
            ...
            process.Dispose();
            throw new InvalidOperationException("No se pudo registrar la sesión Scrcpy.");
        }
    ...
    public async Task<Process> RestartWithProfileAsync(DeviceInfo device, MonitorInfo monitor, OutputProfile profile, bool audioEnabled = true, CancellationToken cancellationToken = default)
    {
        ...
        ValidateProfile(profile);
        var serial = device.Serial.Trim();
        await StopAsync(serial, cancellationToken);
    ...
    private Process StartProcess(DeviceInfo device, MonitorInfo monitor, OutputProfile profile, bool audioEnabled)
    {
        ...
        startInfo.ArgumentList.Add("--disable-screensaver");

        const int windowMargin = 24;
        var availableWidth = Math.Max(320, monitor.WindowWidth - (windowMargin * 2));
        var availableHeight = Math.Max(240, monitor.WindowHeight - (windowMargin * 2));
        var windowWidth = Math.Max(320, (int)(availableWidth * 0.90));
        var windowHeight = Math.Max(240, (int)(availableHeight * 0.90));
        var windowX = monitor.WindowLeft + ((monitor.WindowWidth - windowWidth) / 2);
        var windowY = monitor.WindowTop + ((monitor.WindowHeight - windowHeight) / 2);
    ...
    private void RemoveFinishedSession(string serial)
    {
        if (!_sessions.TryRemove(serial, out var session)) return;
    ...
    private static void ValidateMonitor(MonitorInfo monitor)
    {
        if (monitor is null) throw new InvalidOperationException("No se pudo determinar el monitor de salida.");
    }

    private static void ValidateProfile(OutputProfile profile)
    {
        if (profile is null) throw new InvalidOperationException("No se pudo determinar el perfil de salida.");
    }

    private void ThrowIfDisposed()
    {
        if (_disposed) throw new ObjectDisposedException(nameof(ScrcpyService));
    }
    ...
    public void Dispose()
    {
        ...
        {
            try { if (!pair.Value.Process.HasExited) pair.Value.Process.Kill(entireProcessTree: true); } catch { }
