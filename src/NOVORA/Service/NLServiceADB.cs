...
public sealed class NLServiceADB
{
    ...
    private IReadOnlyList<NLModelDeviceInfo>? _deviceCache;
    private DateTimeOffset _deviceCacheAt =
        DateTimeOffset.MinValue;

    private bool _serverStarted;

    public NLServiceADB(NLServiceNovoraPaths paths)
    {
        _paths = paths;
    }
    ...
    /// </summary>
    private async Task<NLModelDeviceCapabilities> GetDeviceCapabilitiesAsync(
        string serial,
        IReadOnlyList<NLModelDisplayModeInfo> detectedModes,
        CancellationToken cancellationToken)
    {
        int nativeWidth = 0;
        int nativeHeight = 0;
    ...
    private async Task<IReadOnlyList<NLModelDisplayModeInfo>>
    GetDisplayModesAsync(
        string serial,
        CancellationToken cancellationToken)
    {
        var modes =
            new HashSet<NLModelDisplayModeInfo>();
    ...
    /// <summary>Installer-only execution: cancellation stops this ADB client, never the shared ADB server.
    /// Android may already be committing an installation; the caller must report an unknown outcome.</summary>
    public async Task<string> ExecuteAndroidInstallerCommandAsync(string[] arguments, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        _paths.ValidateRequiredTools();
        using var process = Process.Start(CreateStartInfo(arguments))
            ?? throw new InvalidOperationException("No fue posible iniciar ADB.");
        ...
        {
            try { if (!process.HasExited) process.Kill(); } catch (Exception) { /* Preserve the cancellation result even if Windows already closed the process. */ }
            try { await process.WaitForExitAsync().WaitAsync(TimeSpan.FromSeconds(5)); } catch (Exception) { }
