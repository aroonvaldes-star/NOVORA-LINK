...
/// </summary>
public sealed record NLServiceDeviceFastPerformanceResult(
    NLServiceDeviceMetricsSnapshot Before,
    NLServiceDeviceMetricsSnapshot After,
    long MemoryDeltaKb,
    long StorageDeltaKb,
    double LoadDelta,
...
/// </summary>
public sealed class NLServiceDeviceFastPerformance
{
    private readonly NLServiceADB _adb;
    private readonly NLServiceDeviceMetrics _metrics;

    public NLServiceDeviceFastPerformance(
        NLServiceADB adb,
        NLServiceDeviceMetrics metrics)
    {
        _adb =
            adb ??
    ...
    public async Task<NLServiceDeviceFastPerformanceResult> OptimizeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        NLServiceSystemLoadSnapshot loadBefore =
            await GetSystemLoadAsync(
                device.Serial,
                cancellationToken);

        NLServiceStorageSnapshot storageBefore =
        ...
        NLServiceSystemLoadSnapshot loadAfter =
            await GetSystemLoadAsync(
                device.Serial,
                cancellationToken);

        NLServiceStorageSnapshot storageAfter =
            await GetStorageAsync(
                device.Serial,
                cancellationToken);
        ...
        return new NLServiceDeviceFastPerformanceResult(
            before,
            after,
            memoryDeltaKb,
            storageDeltaKb,
    ...
    private async Task<NLServiceSystemLoadSnapshot> GetSystemLoadAsync(
        string serial,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new NLServiceSystemLoadSnapshot(
                load1,
                load5,
                load15);
        }
    ...
    private async Task<NLServiceStorageSnapshot> GetStorageAsync(
        string serial,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (string line in lines.Reverse())
            {
                ...
                return new NLServiceStorageSnapshot(
                    total,
                    used,
                    available);
            }
    ...
    private sealed record NLServiceSystemLoadSnapshot(
        double Load1,
        double Load5,
        double Load15);

    private sealed record NLServiceStorageSnapshot(
        long TotalKb,
        long UsedKb,
        long AvailableKb);
}
