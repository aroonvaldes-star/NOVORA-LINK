...
/// </summary>
public sealed record NLServiceNovoraUpdateInfo(
    Version Version,
    string TagName,
    string ReleaseName,
    string AssetName,
    ...
    string? InstalledVersion = null)
{
    ...
    /// </summary>
    public bool Available => NLServiceReleaseVersion.ShouldOffer(
        InstalledVersion ?? NLServiceReleaseVersion.Installed, TagName);

    public string LatestVersion => NLServiceReleaseVersion.PublicLabel(Version);
}
/// <summary>
/// Comprueba, descarga y prepara la instalación de actualizaciones
/// oficiales de NOVORA.
/// </summary>
public sealed class NLServiceUpdate
{
    ...
    private Task<NLServiceNovoraUpdateInfo?>? _startupCheck;

    public NLServiceUpdate() : this(SharedHttp, NLServiceReleaseVersion.Installed) { }

    internal NLServiceUpdate(HttpClient http, string installedVersion)
    {
        Http = http;
        _installedVersion = installedVersion;
    }

    public Version CurrentVersion => ParseVersion(_installedVersion);

    // Cache the startup request, including failures: no timers or automatic retries.
    public Task<NLServiceNovoraUpdateInfo?> CheckForUpdatesAsync(CancellationToken cancellationToken = default)
    {
        lock (_startupGate)
            return _startupCheck ??= CheckForUpdateAsync(cancellationToken);
    }
    // ============================================================
    // COMPROBAR ACTUALIZACIÓN
    // ============================================================

    public async Task<NLServiceNovoraUpdateInfo?> CheckForUpdateAsync(
        CancellationToken cancellationToken = default)
    {
        ...
        if (!NLServiceReleaseVersion.ShouldOffer(_installedVersion, tag)) return null;
        ...
        var releaseUrl = $"https://github.com/{Repository}/releases/tag/{Uri.EscapeDataString(tag)}";
        // --------------------------------------------------------
        // ASSETS
        // --------------------------------------------------------
        ...
        foreach (var asset in assets.EnumerateArray())
        {
            ...
            var isSetup = IsOfficialInstallerName(assetName);
            if (!isSetup) continue;
            // ----------------------------------------------------
            // HTTPS obligatorio
            // ----------------------------------------------------
            ...
            if (!Uri.TryCreate(
                    ...
                    out var downloadUri) ||
                downloadUri.Scheme !=
                    Uri.UriSchemeHttps || !IsOfficialAssetUri(downloadUri, tag, assetName))
            {
                continue;
            }
            ...
            return new NLServiceNovoraUpdateInfo(
                version,
                tag,
                releaseName,
                assetName,
    ...
    public async Task<string> DownloadInstallerAsync(
        NLServiceNovoraUpdateInfo update,
...
";
    ...
    private static Version ParseVersion(string tag) =>
        NLServiceReleaseVersion.TryParse(tag, out var version, out _) ? version : new Version(0, 0, 0, 0);

    internal static bool IsOfficialInstallerName(string name)
    {
        if (Path.GetFileName(name) != name || name.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0 ||
            name.Contains("arm", StringComparison.OrdinalIgnoreCase) || name.Contains("x86", StringComparison.OrdinalIgnoreCase))
            return false;
        return (name.StartsWith("NOVORA-Setup-", StringComparison.OrdinalIgnoreCase) && name.EndsWith(".exe", StringComparison.OrdinalIgnoreCase)) ||
            (name.StartsWith("NOVORA-LINK-", StringComparison.OrdinalIgnoreCase) &&
             (name.EndsWith("-Setup-x64.exe", StringComparison.OrdinalIgnoreCase) || name.EndsWith("_Setup.exe", StringComparison.OrdinalIgnoreCase)));
    }

    private static bool IsOfficialAssetUri(Uri uri, string tag, string name) =>
        uri.Scheme == Uri.UriSchemeHttps && uri.IsDefaultPort && string.IsNullOrEmpty(uri.UserInfo) &&
        uri.Host.Equals("github.com", StringComparison.OrdinalIgnoreCase) &&
        uri.AbsolutePath.Equals($"/{Repository}/releases/download/{Uri.EscapeDataString(tag)}/{Uri.EscapeDataString(name)}", StringComparison.Ordinal) &&
