...
public sealed class CollectorMetricsLE
{
    private readonly ConcurrentDictionary<string, DeviceMetricsLE> _devices =
        new(StringComparer.OrdinalIgnoreCase);

    public DeviceMetricsLE GetOrCreate(string serial)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(serial);
        return _devices.GetOrAdd(serial, static value => new DeviceMetricsLE(value));
    }

    public DeviceMetricsSnapshotLE GetSnapshot(string serial)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(serial);
        return GetOrCreate(serial).Snapshot();
    }

    public IReadOnlyList<DeviceMetricsSnapshotLE> GetAllSnapshots()
    {
...
public sealed class DeviceMetricsLE
{
    private readonly object _sync = new();

    private StatesCoreLE _state = StatesCoreLE.Disconnected;
    ...
    public DeviceMetricsLE(string serial)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(serial);
        Serial = serial;
    }

    public void SetState(StatesCoreLE state)
    {
        lock (_sync)
        {
            _state = state;
    ...
    public DeviceMetricsSnapshotLE Snapshot()
    {
        lock (_sync)
        {
            bool healthy =
                _state != StatesCoreLE.Failed &&
                _state != StatesCoreLE.Degraded &&
                _latencyMs < 250;

            return new DeviceMetricsSnapshotLE(
                Serial,
                _state,
                _internetActive,
                _bytesSent,
...
public sealed record DeviceMetricsSnapshotLE(
    string Serial,
    StatesCoreLE State,
    bool InternetActive,
    long BytesSent,
