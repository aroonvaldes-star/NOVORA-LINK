...
public sealed class LETransportManager : IAsyncDisposable
{
    public event EventHandler<LETransportSession>? SessionChangedLE;

    private static readonly TimeSpan HandshakeTimeoutLE =
        TimeSpan.FromSeconds(8);

    private readonly ConcurrentDictionary<string, LETransportSession> _sessions =
        new(StringComparer.OrdinalIgnoreCase);

    private readonly ConcurrentDictionary<string, SemaphoreSlim> _sessionLocks =
        new(StringComparer.OrdinalIgnoreCase);

    private readonly ConcurrentDictionary<string, LETransportListener> _listeners =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    private readonly ConcurrentDictionary<string, TaskCompletionSource<LETransportSession>>
        _handshakeWaiters =
            new(StringComparer.OrdinalIgnoreCase);

    private readonly LEMetricsCollector _metrics;
    private readonly LETransportPort _portAllocator;

    private NLServiceADB? _adbService;

    private bool _initialized;
    private bool _disposed;

    public LETransportManager(
    ...
    public Task<LECoreResult> InitializeAsync(
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(
            LECoreResult.Ok(
                "LETransportManager inicializado."));
    }

    public async Task<LECoreResult> OpenAsync(
        string serial,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposedLE();
        ...
        if (!_initialized)
        {
            return LECoreResult.Fail(
                "LETransportManager no está inicializado.");
        }
        ...
        try
        {
            ...
            if (_sessions.TryGetValue(
                    serial,
                    out LETransportSession? currentSession))
            {
                bool reusable =
                    currentSession.State is
                        LETransportState.Ready or
                        LETransportState.Connected or
                ...
                if (reusable)
                {
                    ...
                    return LECoreResult.Ok(
                        $"LETransportManager reutiliza el canal de {serial}. " +
            ...
            const int devicePort =
                LETransportPort.DefaultStartPortLE;
            ...
            if (_adbService is null)
            {
                await FailAndCleanupLEAsync(
                        serial,
                        session,
                        "LETransportManager inició LETransportListener, pero ADB no está enlazado.")
                    .ConfigureAwait(false);
            ...
            session =
                ...
                {
                    State =
                        LETransportState.ReverseConfigured,
            ...
            {
                LETransportSession failedSession =
                    GetSessionLE(
                        serial) ??
                    session;
            ...
            LETransportSession? verifiedSession =
                GetSessionLE(
                    serial);
            ...
            if (verifiedSession is null)
            {
                ...
                return LECoreResult.Fail(
                    "LETransportManager perdió la sesión después de verificar adb reverse.");
            }
                    ...
                    serial))
            {
                await FailAndCleanupLEAsync(
                        serial,
                        verifiedSession,
                        $"LETransportListener dejó de escuchar en el HostPort {hostPort}.")
                    .ConfigureAwait(false);
            ...
            return LECoreResult.Ok(
                $"LETransportManager READY. " +
                $"Android tcp:{devicePort} -> Windows tcp:{hostPort}; " +
                $"LETransportListener LISTENING en 127.0.0.1:{hostPort}; " +
        ...
        {
            LETransportSession? cleanupSession =
                GetSessionLE(
                    serial);
            ...
            return LECoreResult.Fail(
                $"LETransportManager no pudo abrirse: {ex.Message}");
        }
        finally
        {
            sessionLock.Release();
    ...
    public async Task<LECoreResult> VerifyAsync(
        string serial,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposedLE();
        ...
        {
            return LECoreResult.Fail(
                "LETransportManager no tiene un NLServiceADB configurado.");
        }

        if (!_sessions.TryGetValue(
                serial,
                out LETransportSession? session))
        {
            return LECoreResult.Fail(
                $"No existe una sesión LETransportManager para {serial}.");
        }
        ...
        try
        {
            ...
            if (!verified)
            {
                LETransportSession failed =
                    session with
                    {
                        State = LETransportState.Failed,
            ...
            LETransportSession verifiedSession =
                session with
                {
                    ...
                    LastError =
                        listenerActive
                            ? null
                            : "adb reverse existe, pero LETransportListener no está escuchando."
                };
    ...
    public async Task<LECoreResult> WaitForHandshakeAsync(
        string serial,
        TimeSpan timeout,
        CancellationToken cancellationToken = default)
    {
        ...
        TaskCompletionSource<LETransportSession> waiter =
    ...
    public Task<LECoreResult> InjectClientDisconnectLEAsync(
        string serial,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposedLE();
                ...
                out LETransportSession? session))
        {
            return Task.FromResult(
                LECoreResult.Fail(
                    $"No existe sesión LETransportManager para {serial}."));
        }
        ...
        LETransportSession degraded =
            session with
            {
                State = LETransportState.Degraded,
        ...
        return Task.FromResult(
            LECoreResult.Ok(
                ...
                "TcpClient cerrado; adb reverse y LETransportListener permanecen activos."));
    }

    public async Task<LECoreResult> CloseAsync(
        string serial,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposedLE();
        ...
        try
        {
                    ...
                    out LETransportSession? session))
            {
                ...
                return LECoreResult.Ok(
                    "LETransportManager ya estaba cerrado.");
            }
        ...
        string serial)
    {
        ...
        return session.State is
            LETransportState.Preparing or
            LETransportState.ReverseConfigured or
            LETransportState.Listening or
            LETransportState.Ready or
            LETransportState.Connected or
            LETransportState.Degraded;
    }
        ...
        string serial)
    {
        ...
        return
            _listeners.TryGetValue(
                serial.Trim(),
                out LETransportListener? listener) &&
            listener.IsListeningLE;
    }

    public LETransportSession? GetSessionLE(
        string serial)
    {
        if (string.IsNullOrWhiteSpace(serial))
        {
    ...
    public IReadOnlyCollection<LETransportSession> GetSessionsLE()
    {
        return _sessions
            .Values
            .OrderBy(
                        ...
                        cancellationToken))
        {
            ...
            await LETransportHandshake
                .WriteFrameLEAsync(
                    ...
                    $"{LETransportHandshake.ProtocolVersionLE}|ACK",
                    handshakeCts.Token)
                .ConfigureAwait(false);
            ...
            LETransportSession connected =
                session with
                {
                    State = LETransportState.Connected,
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
                            ...
                            cancellationToken))
            {
                heartbeatCts.CancelAfter(
                    LEProtocolHeartbeat.HealthTimeoutLE);
            ...
            if (!LEProtocolHeartbeat.TryParseHeartbeatLE(
                    payload,
                    out long sequence,
            ...
            string ack =
                LEProtocolHeartbeat.CreateAckPayloadLE(
                    sequence,
        ...
        string serial)
    {
        ...
        LETransportSession disconnected =
            session with
            {
                State =
                    listenerActive
    ...
    private static TaskCompletionSource<LETransportSession>
        CreateHandshakeWaiterLE()
    {
        return new TaskCompletionSource<LETransportSession>(
        ...
        string serial)
    {
        ...
        if (_handshakeWaiters.TryRemove(
                serial,
                out TaskCompletionSource<LETransportSession>? waiter))
        {
            waiter.TrySetCanceled();
        }
    }

    private async Task<LECoreResult> StartListenerLEAsync(
        string serial,
        int port,
        CancellationToken cancellationToken)
    {
        if (_listeners.TryGetValue(
                serial,
                out LETransportListener? existing))
        {
            if (existing.IsListeningLE &&
                existing.PortLE == port)
            {
                return LECoreResult.Ok(
                    $"LETransportListener ya escucha en 127.0.0.1:{port}.");
            }
        ...
        try
        {
                    ...
                    listener))
            {
                ...
                return LECoreResult.Fail(
                    $"No se pudo registrar LETransportListener para {serial}.");
            }

            return LECoreResult.Ok(
                $"LETransportListener LISTENING en 127.0.0.1:{port}.");
        }
        catch (OperationCanceledException)
        {
            await listener.DisposeAsync()
        ...
        catch (Exception ex)
        {
            ...
            return LECoreResult.Fail(
                $"LETransportListener no pudo escuchar: {ex.Message}");
        }
    }
    ...
    private async Task<LECoreResult> ConfigureReverseLEAsync(
        string serial,
        int devicePort,
        int hostPort,
        CancellationToken cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        _sessions[serial] =
            session with
            {
                State = LETransportState.Closing,
                UpdatedAtUtc = DateTimeOffset.UtcNow
            };
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        LETransportSession[] sessions =
            _sessions.Values.ToArray();

        foreach (LETransportSession session in sessions)
        {
            if (session.ReverseConfigured &&
                _adbService is not null)
            {
