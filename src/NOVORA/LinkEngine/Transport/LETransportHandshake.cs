...
public static class LETransportHandshake
{
    ...
    public static bool TryParseHelloLE(
        string payload,
        out LETransportHandshakeHello? hello,
        out string? error)
    {
        hello = null;
        error = null;
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            length > MaxPayloadLengthLE)
        {
            throw new InvalidDataException(
                $"LETransportHandshake recibió una longitud inválida: {length}.");
        }
        ...
        string payload)
    {
            ...
            payloadBytes.Length > MaxPayloadLengthLE)
        {
            throw new InvalidOperationException(
                "LETransportHandshake generó un payload fuera del límite permitido.");
        }
               ...
               buffer.Length)
        {
            ...
            if (read == 0)
            {
                throw new EndOfStreamException(
                    "LETransportHandshake perdió el canal antes de recibir el frame completo.");
            }
...
public sealed record LETransportHandshakeHello(
    string Protocol,
    int Version,
    string ClientId);