...
public sealed class ManagerDeviceLE : IAsyncDisposable
{
    private readonly AdbService _adb;
    private readonly CollectorMetricsLE _metrics;

    private readonly ConcurrentDictionary<string, SessionDeviceLE> _sessions =
        new(StringComparer.OrdinalIgnoreCase);

    private bool _initialized;
    private bool _disposed;

    public ManagerDeviceLE(
        AdbService adb,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!_initialized)
            return ResultCoreLE.Fail("ManagerDeviceLE no está inicializado.");

        if (string.IsNullOrWhiteSpace(serial))
            return ResultCoreLE.Fail("Serial ADB inválido.");
        ...
        try
        {
                ...
                when (ContainsOffline(ex.Message))
            {
                ...
                _metrics
                    .GetOrCreate(serial)
                    .SetState(StatesCoreLE.Degraded);
                    ...
                    StringComparison.OrdinalIgnoreCase))
            {
                ...
                _metrics
                    .GetOrCreate(serial)
                    .SetState(
                        deviceState == StateDeviceLE.Offline
            ...
            return ResultCoreLE.Ok(
                $"ManagerDeviceLE conectado: {serial} | {connectionType} | ADB Online.");
        }
        ...
        catch (Exception ex)
        {
            ...
            return ResultCoreLE.Fail(
                $"ManagerDeviceLE no pudo validar {serial}.",
                ex);
        }
    }

    public async Task<ResultCoreLE> RefreshAsync(
        string serial,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            return online
                ? ResultCoreLE.Ok(
                    $"ManagerDeviceLE saludable: {serial} | {updated.ConnectionType}.")
                : ResultCoreLE.Fail(
                    $"ManagerDeviceLE detectó {serial} offline.");
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        catch (Exception ex)
        {
            ...
            return ResultCoreLE.Fail(
                $"No fue posible refrescar ManagerDeviceLE para {serial}.",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(serial))
        {
            ...
            _metrics
                .GetOrCreate(serial)
                .SetState(StatesCoreLE.Disconnected);
        }
    ...
    public bool IsConnected(string serial)
    {
        ...
        return _sessions.TryGetValue(
                   serial.Trim(),
                   out var session) &&
               session.State == StateDeviceLE.Online &&
               session.AdbOnline;
    }

    public SessionDeviceLE? GetSession(string serial)
    {
        ThrowIfDisposed();
    ...
    public ConnectionDeviceLE GetConnectionType(string serial)
    {
        ThrowIfDisposed();
    ...
    private static StateDeviceLE ParseAdbState(string state)
    {
        if (string.Equals(
                state,
                "device",
        ...
        if (string.Equals(
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            return StateDeviceLE.Unauthorized;
        }

        return StateDeviceLE.Unknown;
    }

    private static ConnectionDeviceLE DetectConnectionType(string serial)
    {
        /*
         ...
         */
        if (string.IsNullOrWhiteSpace(serial))
            return ConnectionDeviceLE.Unknown;
