...
public sealed class NLControlClient : IAsyncDisposable
{
    ...
    public async Task<NLControlReply> ConnectAsync(string code, int port = NLControlProtocol.Port)
    {
        using var deadline = CancellationTokenSource.CreateLinkedTokenSource(_stop.Token);
        deadline.CancelAfter(TimeSpan.FromSeconds(10));
        await _socket.ConnectAsync(IPAddress.Loopback, port, deadline.Token);
        _socket.NoDelay = true;
            ConfigureLiveness();
        _stream = _socket.GetStream();
        _reader = ReadLoopAsync();
        return await SendAsync("pair", code: code);
    }
