...
public sealed class NLControlTrustServer : IAsyncDisposable
{
    ...
    public NLControlTrustServer(IPAddress address, Func<NLControlRequest, Task<NLControlReply>> handler, NLControlTrustStore store, int port = 27215, bool allowPairing = true)
    {
    ...
    private async Task RunAsync()
    {
        ...
        try
        {
            while (!_stop.IsCancellationRequested)
            {
                ...
                StatusChanged?.Invoke(this, "LAN disponible para dispositivos de confianza.");
            }
        }
        catch (Exception ex) when (ex is OperationCanceledException or SocketException or ObjectDisposedException) { }
        finally { IsClosed = true; _authorized = false; _listener.Stop(); StatusChanged?.Invoke(this, "Control LAN detenido."); }
    }
    private async Task<bool> ServeAsync(TcpClient client)
    {
        ...
        { await NLControlProtocol.WriteAsync(stream, new NLControlReply(1, hello.Id, false, "Autorización rechazada. Vincula de nuevo con QR."), timeout.Token); return false; }
        ...
        StatusChanged?.Invoke(this, resumed ? "Android de confianza conectado por LAN." : "Android autorizado por QR.");
        ...
        try
        {
            ...
            while (!_stop.IsCancellationRequested)
            {
                ...
                {
                    if (!paired || enrollmentUsed) reply = new(1, request.Id, false, "Guardar confianza requiere una nueva invitación QR.");
                    else
                    {
                        try
                        {
                            ...
                            reply = new(1, request.Id, true, "PC guardada como confiable.", TrustedPc: new(Environment.MachineName, _address.ToString(), Port, _store.Fingerprint, grant.Id, grant.Token));
                        }
