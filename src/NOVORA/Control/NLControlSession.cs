...
public sealed class NLControlSession : IAsyncDisposable
{
    ...
    public Task ConnectUsbAsync(NLControlLanInvitation invitation)
    {
        invitation.Validate(true);
        if (invitation.Host != "127.0.0.1" || invitation.Port != NLControlProtocol.Port) throw new InvalidDataException("Destino USB no válido.");
