...
VECoreResult start = await engine.StartAsync(serial, options, stopCts.Token);
if (!start.Success)
{
...
VECoreStatus status = engine.StatusVE;
...
VECoreResult stop = await engine.StopAsync();
if (!stop.Success)
{
...
if (!status.ControlReady && control.State is not VEControlStates.EndOfStream)
{
    Console.Error.WriteLine("FAIL: ControlVE no llegó a estado READY.");
    return 8;
}

if (audio.State == VEAudioStates.Failed || status.AudioDecodeErrors != 0)
{
