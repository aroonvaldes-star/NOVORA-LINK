...
public sealed class NLTestAndroidFiles : IDisposable
{
    ...
    public async Task AuthenticatedSocketTransferPublishesExactFile()
    {
        ...
        await using var server = new NLTestControlTrustHarness(async request =>
            new NLControlReply(1, request.Id, true, request.Action == "pair" ? "paired" : await receiver.HandleAsync(request.Action, request.Value), State()));
        await using var client = new NLControlClient();
        Assert.True((await server.ConnectAsync(client).WaitAsync(TimeSpan.FromSeconds(5))).Success);
    ...
    public async Task UnpairedSocketCannotBeginTransfer()
    {
        ...
        { called = true; return Task.FromResult(new NLControlReply(1, request.Id, true, "unexpected")); });
        using var socket = new TcpClient();
        using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        await socket.ConnectAsync(System.Net.IPAddress.Loopback, server.Server.Port, timeout.Token);
