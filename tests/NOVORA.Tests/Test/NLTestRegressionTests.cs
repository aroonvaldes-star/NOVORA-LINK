...
public sealed class NLTestRegressionTests
{
    [Fact]
    ...
    public void Output_profile_is_limited_by_phone_not_pc_monitor()
    {
        var device = new NLModelDeviceInfo
        {
            ...
            Model = "Phone",
            Connected = true,
            BestDisplayMode = new NLModelDisplayModeInfo(1080, 2400, 120),
            SupportedDisplayModes = new[] { new NLModelDisplayModeInfo(1080, 2400, 120) }
        };
        var monitor = new NLModelMonitorInfo(
            "DISPLAY1",
            "Monitor 1",
        ...
        var profile = new NLServiceOutputProfile().Calculate(
            device,
            monitor,
            "10M",
            targetFps: 120,
    ...
    {
        var closingHandler = typeof(NLUIWindowSettings).GetMethod(
    ...
    public void Adb_validation_does_not_require_scrcpy()
    {
        ...
        try
        {
            var paths = new NLServiceNovoraPaths(root);
    ...
    public void Cpu_parser_handles_android_top_summary_format()
    {
        ...
        var method = typeof(NLServiceDeviceMetrics).GetMethod(
            "ParseCpu",
    ...
    {
        var assembly = typeof(NLModelDeviceInfo).Assembly;
        var capabilitiesType = assembly.GetType("NOVORA.Model.NLModelDeviceCapabilities");
        Assert.NotNull(capabilitiesType);

        var capabilitiesProperty = typeof(NLModelDeviceInfo).GetProperty("Capabilities");
        ...
        var viewModel = new NLViewModelMain
        {
            TargetFps = 240,
            MaxSize = 5120,
            Device = device
    ...
    {
        var type = typeof(NLServiceDeviceState);
