...
public sealed class NLTestAndroidLan
{
    ...
    [Fact]
    public void InvitationRoundTripsWithoutLosingPinOrSecret()
    ...
    {
        int invoked = 0;
        await using var server = new NLControlLanServer(IPAddress.Loopback, request =>
        { Interlocked.Increment(ref invoked); return Task.FromResult(new NLControlReply(1, request.Id, true, "OK", State)); }, 0);
        server.Start();
    ...
    [Fact]
    public async Task WrongPinFailsBeforeAnyHandlerInvocation()
    {
        int invoked = 0;
        await using var server = new NLControlLanServer(IPAddress.Loopback, request =>
        { Interlocked.Increment(ref invoked); return Task.FromResult(new NLControlReply(1, request.Id, true, "OK", State)); }, 0);
